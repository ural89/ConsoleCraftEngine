...
void textbackground(short kolor) {
	...
	SetConsoleTextAttribute(hConsole, (csbi.wAttributes & 0xFFF0) | kolor);
}
...
void textcolor(short kolor) {
	...
	SetConsoleTextAttribute(hConsole, (csbi.wAttributes & 0xFF0F) | (kolor << 4));
}
...
void window(int xup, int yup, int xdown, int ydown) {
	...
	SMALL_RECT windowSize = { xup - 1, yup - 1, xdown - 1, ydown - 1 };
...
bool kbhit() {
	...
	PeekConsoleInput(hConsole, &inRec, 1, &numEvents);

	return inRec.EventType == KEY_EVENT && inRec.Event.KeyEvent.bKeyDown;
}
...
int getch() {
	...
	while (true) {
		ReadConsoleInput(hConsole, &ir, 1, &numRead);
...
char* cgets(char* str) { // nie wiem dokladnie jak dziala orginalna f. cgets bo nie mam
	// do niej referencji..
	if (str == NULL || *str == 0) {
		*(str + 1) = 0;
		return NULL;
...
void window(int xup, int yup, int xdown, int ydown) {
	if (xup<1 || yup<1 || xdown>COLS || ydown>LINES) { //jesli zle dane podano...
		xdown = COLS - xup;
		ydown = LINES - yup;
		//return;
	}
